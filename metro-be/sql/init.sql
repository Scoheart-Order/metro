...
ALTER TABLE announcements ADD CONSTRAINT fk_announcements_admin_id FOREIGN KEY (admin_id) REFERENCES users(id);
ALTER TABLE feedback ADD CONSTRAINT fk_feedback_user_id FOREIGN KEY (user_id) REFERENCES users(id);
ALTER TABLE request ADD CONSTRAINT fk_request_user_id FOREIGN KEY (user_id) REFERENCES users(id);
ALTER TABLE reply ADD CONSTRAINT fk_reply_user_id FOREIGN KEY (user_id) REFERENCES users(id);

-- 创建初始管理员账号
INSERT INTO users (username, password, enabled, real_name, email, role)
VALUES ('admin', '$2a$10$9Wr/oFKsZpHQATNRkZ1d6eCDErGxN7pKP4/tpMaQfpYiXcpRmWIgu', TRUE, '系统管理员', 'admin@metro.com', 'ROLE_ADMIN');
